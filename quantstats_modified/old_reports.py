...
def html(returns, benchmark=None, rf=0., grayscale=False,
         ...
         figfmt='svg', template_path=None, match_dates=False, start_date=None, end_date=None, filepath=None):
    ...
    if start_date<datetime.strptime(date_range[0], "%d %b, %Y"):
        start_date = datetime.strptime(date_range[0], "%d %b, %Y")
    if end_date>datetime.strptime(date_range[-1], "%d %b, %Y"):
        end_date = datetime.strptime(date_range[-1], "%d %b, %Y")
    
    date_range_converted = [datetime.strptime(date, "%d %b, %Y") for date in date_range]
        
    date_range = [date_range[i] for i, date in enumerate(date_range_converted) if date>start_date and date<end_date]
        

    tpl = tpl.replace('{{date_range}}', start_date.strftime("%d %b, %Y") + ' - ' + end_date.strftime("%d %b, %Y"))
    ...
    returns = returns.loc[start_date.strftime("%Y-%m-%d"):end_date.strftime("%Y-%m-%d")]
    _plots.returns(returns, benchmark, grayscale=grayscale,
                   ...
                   prepare_returns=False, filepath=filepath)
    ...
    _plots.rolling_volatility(returns, benchmark, grayscale=grayscale,
                              ...
                              periods_per_year=win_year, filepath=filepath)
    ...
    _plots.drawdown(returns, grayscale=grayscale,
                    figsize=(8, 3), subtitle=False,
                    savefig={'fname': figfile, 'format': figfmt},
                    show=False, ylabel=False, filepath=filepath)
    ...
    _plots.monthly_heatmap(returns, grayscale=grayscale,
                           ...
                           savefig={'fname': figfile, 'format': figfmt},
                           show=False, ylabel=False, compounded=compounded, filepath=filepath)
