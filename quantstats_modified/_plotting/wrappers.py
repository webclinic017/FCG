...
def savefile(data, benchmark, filepath, filename):
    if not os.path.exists(filepath): os.makedirs(filepath)
    if not isinstance(benchmark, type(None)):
        min_length = min([len(data.index), 
                      len(data.values), 
                      len(benchmark.values),])
        df = pd.DataFrame.from_dict({"Date": data.index[:min_length],
                                    data.name: data.values[:min_length],
                                    data.name+"_benchmark": benchmark.values[:min_length],
                                    })
...
def snapshot(returns, grayscale=False, figsize=(10, 8),
    ...
    savefile(returns, None, filepath, "snapshot.csv")
...
def earnings(returns, start_balance=1e5, mode="comp",
             ...
             subtitle=True, savefig=None, show=True, filepath=None):
    ...
    savefile(returns, None, filepath, "earnings.csv")
...
def returns(returns, benchmark=None,
            ...
            prepare_returns=True, filepath=None):
    ...
    savefile(returns, benchmark, filepath, "returns.csv")
...
def log_returns(returns, benchmark=None,
    ...
    savefile(returns, benchmark, filepath, "log_returns.csv")
...
def daily_returns(returns,
    ...
    savefile(returns, None, filepath, "daily_returns.csv")
...
def yearly_returns(returns, benchmark=None,
    ...
    savefile(returns, benchmark, filepath, "yearly_returns.csv")
...
def distribution(returns, fontname='Arial', grayscale=False, ylabel=True,
    ...
    savefile(returns, None, filepath, "distribution.csv")
...
def histogram(returns, resample='M', fontname='Arial',
    ...
    savefile(returns, None, filepath, "histogram.csv")
...
def drawdown(returns, grayscale=False, figsize=(10, 5),
             ...
             resample=None, subtitle=True, savefig=None, show=True, filepath=None):

    dd = _stats.to_drawdown_series(returns)

    savefile(returns, None, filepath, "drawdown.csv")
...
def drawdowns_periods(returns, periods=5, lw=1.5, log_scale=False,
    ...
    savefile(returns, None, filepath, "drawdowns_periods.csv")
...
def rolling_beta(returns, benchmark,
    ...
    savefile(returns, None, filepath, "rolling_beta.csv")
...
def rolling_volatility(returns, benchmark=None,
    ...
    savefile(returns, benchmark, filepath, "rolling_volatility.csv")
...
def rolling_sharpe(returns, benchmark=None, rf=0.,
    ...
    savefile(returns, benchmark, filepath, "rolling_sharpe.csv")
...
def rolling_sortino(returns, benchmark=None, rf=0.,
    ...
    savefile(returns, benchmark, filepath, "rolling_sortino.csv")
